...
//      _cupertinoPickerBloc.inputSelectBrand.add(widget.product.brandId);
//      _cupertinoPickerBloc.inputSelectSize.add(widget.product.sizeId);
//      _cupertinoPickerBloc.inputSelectType.add(widget.product.typeId);
                ...
                children: <Widget>[
                  ...
                  new StreamBuilder(
                    ...
                    builder: (context, snapshot) => new RaisedButton(
                      ...
                      onPressed: snapshot.data == ButtonState.LOADING ? null :
                          () {
                        onButtonPressed();
                      },
                    ),
    ...
    if(!_cupertinoPickerBloc.selectedBrand.isEmpty()
      ...
      _keyScaffold.currentState.showSnackBar(
          SnackbarBuilder.getSnackbar(
              "Pastikan merk, type, dan size tidak kosong",
              StringConstant.OK));
    }
  }
        ...
        if (snapshot.hasData) {
          if (isRequestSuccess(snapshot.data)) {
            return new GestureDetector(
              onTap: snapshot.data.data.isEmpty ? null : () async{
              ...
              child: _buildMenu(<Widget>[
                ...
                new StreamBuilder<CupertinoData>(
                  ...
                  builder: (context, snapshotSelectedItem) =>
                      buildSelectedCupertinoItemText(snapshotSelectedItem.data.information),
                ),
              ]),
            );
          } else if (snapshot.data.result ==
              ResponseDatabase.ERROR_SHOULD_RETRY) {
            return buildRetryWidget(_cupertinoPickerBloc
        ...
        if (snapshot.hasData) {
          if (isRequestSuccess(snapshot.data)) {
            return new Expanded(
              flex: 2,
              child: FlatButton(
                onPressed: snapshot.data.data.isEmpty ? null :() async {
                  await setupCupertinoModal(
                      snapshot.data.data,
      ...
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          if (isRequestSuccess(snapshot.data)) {
            return new GestureDetector(
              onTap: snapshot.data.data.isEmpty ? null : () async{
                await setupCupertinoModal(snapshot.data.data,
              ...
              child: _buildMenu(<Widget>[
                ...
                new StreamBuilder<CupertinoData>(
                  ...
                  builder: (context, snapshotSelectedType) {
                    return buildSelectedCupertinoItemText(snapshotSelectedType.data.information);
                  },
                ),
              ]),
            );
  ...
  Widget _buildCupertinoPicker(List<CupertinoData> data,
      StreamSink<int> sinker,
      int initialIndex) {
